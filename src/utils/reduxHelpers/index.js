...
const reduxGenerator = (featureName) => {
  ...
  const reducer = handleActions(
    {
      ...
      [actionTypes.GET_ALL_CLEAN]: (state) => {
        return state.set("items", fromJS([])).set("getAllError", fromJS({}));
      },
      ...
      [actionTypes.INSERT_SUCCEEDED]: (state, { payload: { data } }) => {
      ...
      [actionTypes.UPDATE_SUCCEEDED]: (state) => {
  ...
  const selectors = {
    ...
    insertError: createSelector(select, (f) => {
      return f.get("insertError");
    }),
    isInsertSucceeded: createSelector(select, (f) => {
      return f.get("isInsertSucceeded");
    }),
    isUpdateSucceeded: createSelector(select, (f) => {
      return f.get("isUpdateSucceeded");
    }),
    isDeleteSucceeded: createSelector(select, (f) => {
      return f.get("isDeleteSucceeded");
    }),
    isLoadingItems: createSelector(selectAjax, (ajax) => {
      return ajax.get(`${featureNameUpperCase}_GET_ALL`);
    }),
    isLoadingItem: createSelector(selectAjax, (ajax) => {
      return ajax.get(`${featureNameUpperCase}_GET`) === 1;
    }),
    isInserting: createSelector(selectAjax, (ajax) => {
      return ajax.get(`${featureNameUpperCase}_INSERT`) === 1;
    }),
    isUpdating: createSelector(selectAjax, (ajax) => {
      return ajax.get(`${featureNameUpperCase}_UPDATE`) === 1;
    }),
    isDeleting: createSelector(selectAjax, (ajax) => {
      return ajax.get(`${featureNameUpperCase}_DELETE`) === 1;
    }),
  };
